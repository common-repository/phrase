...
class PhraseAPI {
	...
	public function get_post_keys( string $project_id, int $post_id ) {
		...
		try {
			...
			$all_keys = [];
			while ( \is_int( $page ) ) {
				list($keys, , $headers) = $api->keysListWithHttpInfo(
					$project_id,
					null,
					$page,
					null,
				...
				if ( $pagination ) {
					$pagination = json_decode( $pagination[0], true );
	...
	public function get_translations( string $project_id, array $keys = [] ) {
		...
		try {
			...
			foreach ( $chunks as $chunk ) {
				$q    = 'key.id:' . implode( ',', $chunk );
				$page = 1;
				while ( \is_int( $page ) ) {
					list($translations, , $headers) = $api->translationsListWithHttpInfo(
						$project_id,
						null,
						$page,
						100,
